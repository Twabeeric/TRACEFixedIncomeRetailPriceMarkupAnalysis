...
```{r}
setwd("M:/Documents/R/Detalus/Data1/MBS") 
char_data <- read.csv("MBS2018.csv", stringsAsFactors = F) 
char_data=char_data[,c("rdid","ascii_rptd_vol_tx", "trd_exctn_dt", "trd_exctn_tm","trans_dt","rptd_pr", "side", "rptg_party_type", "contra_party_type" )]#reading in csv file
char_data$ascii_rptd_vol_tx[which(char_data$ascii_rptd_vol_tx=="10MM+")]=10000000
...
statsmatrix=matrix(nrow=7, ncol=1)

rownames(statsmatrix) = c("no of transactions", "par value", "value traded", "par purchases", "par sales", "value purchases", "value sales")
...
setwd("M:/Documents/R/Detalus/Data1/MBS") 
p3 = ggplot(ddxnp, aes(x = ddxnp$ascii_rptd_vol_tx))+geom_point(aes(y=ddxnp$markup, col=ddxnp$side, alpha=.5)) + xlim(c(0, 10000000)) + ylim(c(0, .5)) + labs(subtitle="Volume vs Markups", 
       y="Markups", 
       x="Volume", 
       title="Scatterplot")

plot(p3)
ggsave("MBSBuyVsSell.png")

#Plotting for Dealer Sells
p4 = ggplot(ddxnp[which(ddxnp$side=="S"),], aes(x = ddxnp$ascii_rptd_vol_tx[which(ddxnp$side=="S")],y=ddxnp$markup[which(ddxnp$side=="S")]) )+geom_point(color='darkblue') + xlim(c(0, 10000000)) + ylim(c(0, .5)) + labs(subtitle="Volume vs Markups Dealer Sells", 
       y="Markups", 
       x="Volume", 
       title="Scatterplot") 

plot(p4)
ggsave("MBSVolumeVsMarkupDealerSells.png")

#Plotting for Dealer Buys
p5 = ggplot(ddxnp[which(ddxnp$side=="B"),], aes(x = ddxnp$ascii_rptd_vol_tx[which(ddxnp$side=="B")],y=ddxnp$markup[which(ddxnp$side=="B")]) )+geom_point(color='red') + xlim(c(0, 10000000)) + ylim(c(0, .5)) + labs(subtitle="Volume vs Markups Dealer Buys", 
       y="Markups", 
       x="Volume", 
       title="Scatterplot") 

plot(p5) #plotting volume versus markup for dealer sells for same time trades
ggsave("MBSVolumeVsMarkupDealerBuys.png")

p6=ggplot(data = ddxnp[which(ddxnp$side=="S"),]) + 
  geom_density(mapping = aes(x = ddxnp$markup[which(ddxnp$side=="S")]))+ xlim(c(-0.3,0.3)) + labs(subtitle="Markup vs Density Dealer Sells", 
       y="Density", 
       x="Markup", 
       title="Density plot") 
plot(p6)#plotting markup density for dealer sells for same time trades
ggsave("MBSDensityVolumeVsMarkupDealerSells.png")


p7=ggplot(data = ddxnp[which(ddxnp$side=="B"),]) + 
  geom_density(mapping = aes(x = ddxnp$markup[which(ddxnp$side=="B")]))+ xlim(c(-0.3, 0.3))  + labs(subtitle="Markup vs Density Dealer Buys", 
       y="Density", 
       x="Markup", 
       title="Density plot") 
plot(p7)#plotting markup density for dealer sells for same time trades
ggsave("MBSDensityVolumeVsMarkupDealerbuys.png")
#write.csv(ddxnp[which(ddxnp$markup>.7),], file = "weirdMBSExtreamMarkups")
```
...
```{r}
setwd("M:/Documents/R/Detalus/Data1/MBS") 
lotmatrix=matrix(ncol=6, nrow=12)
colnames(lotmatrix)=c("Min", "1st Qu", "Median", "Mean", "3rd Qu", "Max")
rownames(lotmatrix)=c("B 1K-50K","S 1K-50K","B 50K-100K","S 50K-100K", "B 100K-500K","S 100K-500K","B 500K-1000K","S 100K-500K","B 1MM-5MM","S 1MM-5MM","B 5MM-10MM","S 5MM-10MM")
...
lotmatrix[11,]=summary(ddxnp$markup[which(ddxnp$ascii_rptd_vol_tx>=5000000 & ddxnp$ascii_rptd_vol_tx<10000000 & ddxnp$side=="B")])
lotmatrix[12,]=summary(ddxnp$markup[which(ddxnp$ascii_rptd_vol_tx>=5000000 & ddxnp$ascii_rptd_vol_tx<10000000 & ddxnp$side=="S")])

write.csv(lotmatrix, file = "SummaryAnalysisMBS.csv")
...
ggsave("MBSInterDealerDatesVsPrices.png")


#Plotting for one cusip
p15 = ggplot()+geom_point(aes(x = ddxnpp$trd_exctn_dt[which(ddxnpp$cusip_id=="NSL403C483A02250F")], y=ddxnpp$rptd_pr[which(ddxnpp$cusip_id=="NSL403C483A02250F")]), data=ddxnpp[which(ddxnpp$cusip_id=="NSL403C483A02250F"),], color='red', alpha=.5) + geom_point(aes(x = ddxnpp$trd_exctn_dt[which(ddxnpp$cusip_id=="NSL403C483A02250F")], y=ddxnpp$rptd_pr..10[which(ddxnpp$cusip_id=="NSL403C483A02250F")]), data=ddxnpp[which(ddxnpp$cusip_id=="NSL403C483A02250F"),], shape = 17,colour = 'grey', alpha=.7)+ylim(c(90, 110)) + labs(subtitle="Dates vs Prices", 
       y="Prices", 
       x="Dates", 
       title="Scatterplot for NSL403C483A02250F")
plot(p15)
ggsave("MBSInterDealerDatesVsPricesSingleID.png")
```
Lotmatrix for Interdealer
```{r}
setwd("M:/Documents/R/Detalus/Data1/MBS") 
lotmatrix1=matrix(ncol=6, nrow=12)
colnames(lotmatrix1)=c("Min", "1st Qu", "Median", "Mean", "3rd Qu", "Max")
rownames(lotmatrix1)=c("Int 1K-50K","Oth 1K-50K","Int 50K-100K","Oth 50K-100K", "Int 100K-500K","Oth 100K-500K","Int 500K-1000K","Oth 100K-500K","Int 1MM-5MM","Oth 1MM-5MM","Int 5MM-10MM","Oth 5MM-10MM")
...
lotmatrix1[11,]=summary(ddxnpp$rptd_pr[which(ddxnpp$ascii_rptd_vol_tx>=5000000 & ddxnpp$ascii_rptd_vol_tx<10000000 & ddxnpp$side=="S")])
lotmatrix1[12,]=summary(ddxnpp$rptd_pr..10[which(ddxnpp$ascii_rptd_vol_tx>=5000000 & ddxnpp$ascii_rptd_vol_tx<10000000 & ddxnpp$side=="S")])

write.csv(lotmatrix1, file = "MBSInterdealerSummaryAnalysis.csv")
```

#Write agency trades file to excel with interdealer trades
```{r}
setwd("M:/Documents/R/Detalus/Data1/MBS") 
write.csv(ddxnpp, file = "MBSInterdealerSameday.csv")
write.csv(ddxnpp[which(ddxnpp$side=="B"),], file = "MBSInterdealerBuysSameDay.csv")
write.csv(ddxnpp[which(ddxnpp$side=="S"),], file = "MBSInterdealerSellsSameDay.csv")
